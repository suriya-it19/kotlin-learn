...
// in and out specifiers
// out - to assign reference to any of its supertypes
class Producer<out T>(val value: T) {
    fun get(): T{
        return value
    }
}
val parameterizedProducer = Producer("string")
val ref: Producer<Any> = parameterizedProducer

// in - if we want to assign it to the reference of its subtype
class ProducerT<in T> {
    fun convert(value: T): String {
        return value.toString()
    }
}
val parameterizedConsumer = ProducerT<Number>()
val ref: ProducerT<Double> = parameterizedConsumer
assertTrue(ref is ProducerT<Double>)
